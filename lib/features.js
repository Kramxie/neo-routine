 ...
 * @param {number} additionalCount - Number of tasks being added (default 1)
 * @returns {Object} - { allowed: boolean, limit: number, remaining: number }
 */
export function canAddTask(userTier, currentCount, additionalCount = 1) {
  const limit = getLimit(userTier, 'maxTasksPerRoutine');
  const afterAdd = currentCount + additionalCount;
  const remaining = limit === Infinity ? Infinity : Math.max(0, limit - currentCount);
  return {
...
/**
 * Check if user can adopt more templates
 * @param {string} userTier - User's subscription tier
 * @param {number} currentCount - Current number of template-based routines created
 * @returns {Object} - { allowed: boolean, limit: number, remaining: number }
 */
export function canAdoptTemplate(userTier, currentCount) {
  const limit = getLimit(userTier, 'maxTemplateAdoptions');
  const remaining = limit === Infinity ? Infinity : Math.max(0, limit - currentCount);
  return {
    allowed: currentCount < limit,
    limit,
...
export function getUpgradePrompt(feature) {
  const prompts = {
    ...
    coachAccess: {
      title: 'Premium Coach Templates',
      description: 'Unlock all professionally crafted routines from certified coaches. Upgrade to Premium to access the full template marketplace.',
      cta: 'Upgrade to Premium',
    },
    templateAdoptions: {
      title: 'Template Adoption Limit Reached',
      description: 'Free users can adopt 1 template. Upgrade to Premium to adopt more templates anytime.',
      cta: 'Upgrade to Premium',
    },
    default: {
