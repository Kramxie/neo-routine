...
const createTransporter = () => {
  ...
  return nodemailer.createTransport({
    ...
    family: 4,
    // Reject unauthorized certs in production; allow self-signed in dev
    tls: {
...
export async function sendPaymentFailedEmail(email, name) {
  ...
  try {
    const mailOptions = {
      from: `"Neo Routine" <${process.env.GMAIL_USER}>`,
      to: email,
      subject: 'Action Required: Payment Failed - Neo Routine',
      html: getPaymentFailedEmailTemplate(name),
    };
...
/**
 * Generate payment failed email HTML template
 */
function getPaymentFailedEmailTemplate(name) {
  const APP_URL = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000';
  const updatePaymentUrl = `${APP_URL}/dashboard/upgrade`;
  ...
  return `
    ...
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f5f5f5; margin: 0; padding: 40px 20px;">
      <div style="max-width: 480px; margin: 0 auto; background: white; border-radius: 12px; padding: 40px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        ...
        <!-- Content -->
        <h2 style="color: #1e293b; font-size: 20px; margin-bottom: 16px;">Hi ${name || 'there'},</h2>
        <p style="color: #64748b; font-size: 16px; line-height: 1.6; margin-bottom: 24px;">
          We were unable to process your subscription payment. To continue enjoying your premium features, please update your payment method.
        </p>
        
        <!-- CTA Button -->
        <div style="text-align: center; margin: 32px 0;">
          <a href="${updatePaymentUrl}" style="display: inline-block; background: linear-gradient(135deg, #0ea5e9 0%, #0284c7 100%); color: white; text-decoration: none; padding: 16px 32px; border-radius: 8px; font-weight: 600; font-size: 16px;">
